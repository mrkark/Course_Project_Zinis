...
class BehavioralEmulator {
   ...
   * @returns {Promise<Object>} Emulation result with completionPromise
   */
  async startEmulation(scanId, analysisResults, onEvent = null) {
    ...
    // Create a promise that resolves when emulation finishes
  ...
  runEmulation(scanId, state) {
    const runStage = () => {
      ...
      state.stageTimeoutId = setTimeout(() => {
        ...
        // Переход к следующему этапу после завершения таймера этапа
        state.currentStage++;
        state.currentEventIndex = 0;
        if (state.isRunning) {
          state.stageTimeoutId = setTimeout(runStage, 500);
   ...
   * @param {Function|null} onEvent - Optional event collector
   */
  async emitEvent(scanId, eventData, onEvent = null) {
    // Stream the event to clients. ScanService persists the collected events
    // after the real Scans.id exists.
   ...
   * This method is intentionally simulation-only: no uploaded code is executed.
   */
  finishEmulation(scanId, state) {
    ...
    const result = {
      scanId,
      malwareType: state.profile.name,
      eventCount: state.currentEventIndex,
